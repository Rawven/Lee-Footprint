...
public class b_组合总和2 {
	...
	/**
	 * 给定一个候选人编号的集合 candidates 和一个目标数 target ，找出 candidates 中所有可以使数字和为 target 的组合。 candidates
	 * 中的每个数字在每个组合中只能使用 一次 。 注意：解集不能包含重复的组合。 combination sum2
	 *
	...
	public static void tool(List<List<Integer>> saveL, List<Integer> save,
	                        int initNum, int index, int target, int begin, int len, int[] array,
